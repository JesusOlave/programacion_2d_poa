...
{    
    public class UTGameObject
    {
        ...
        public UTGameObject(string imagen, Vector2 pos, float rot, float escala, FF_form forma, bool isStatic = false, bool isNonFisic = false)
        {
            dibujable = new Dibujable(imagen, pos, escala, rot);
            objetoFisico = new ObjetoFisico(dibujable);
            if (forma == FF_form.Circulo && isNonFisic == false)
            {
                objetoFisico.agregarFFCirculo(dibujable.ancho / 2f, new Vector2(0, 0));
            }
            else if (forma == FF_form.Rectangulo && isNonFisic == false)
            {
                objetoFisico.agregarFFRectangulo(dibujable.ancho, dibujable.alto, new Vector2(0, 0));
            }
            else
            {
                objetoFisico.isNoFisic = true;    
            }
